...
function Writable(opts) {
  ...
  state.toBuffer = !opts.toBuffer && !opts.writeableToBuffer;
    ...
    if (state.afterPipe) {
      ...
      cb(
        err ||
          (state.finishEmitted
            ? null
            : state.destroyReason || new Error("Stream closed prematurely"))
      );
    }
...
Writable.prototype.write = function (data) {
  var state = this._writableState;
  if (!state.objectMode && typeof data === "string") {
    data = Buffer.from(data);
  }
  return writeMaybe(this, state, data);
};
...
Writable.prototype.destroy = Writable.prototype.destroyWritable = function (
  err
) {
  var state = this._writableState;
...
Writable.prototype.destroyMaybe = function () {
  if (this._writableState.finishEmitted) {
    this.destroy();
  }
};
